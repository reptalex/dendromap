...
depthDraw <- function(d1,d2,lambda){
  ...
  sampler <- function(col.node,dT,lambda){
    if (all(lambda==-Inf)){
      return(as.integer(0))
    } else {
        ...
        if (any(is.infinite(lambda))){
          ...
          if (all(lambda[is.infinite(lambda)]<0)){
            ...
            if (length(lambda)==0){
              ...
              cs <- sample(col.node[ix],1,prob=lambda)
            }
            ...
            if (sum(vals)==1){
              ...
              cs <- sample(col.node[vals],1,prob=rep(1,sum(vals)))
            }
          }
        } else {
          cs <- sample(col.node,1,prob=lambda)
        }
  ...
  D[,list(col=sampler(col.node,dT,lambda)),by=row] %>%
  return()
}
